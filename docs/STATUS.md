...
| 0053e | Cloud.AI | The shared token retired | **done** | **Proved on the deployment on 11 September 2026.** A person signed in through the browser — password, the installer password replaced, a second factor enrolled from a QR code and confirmed — and the shared token presented afterwards was refused with 401, as was a request carrying nothing. `IsOperator` takes a session and nothing else; the header, both token boxes and `OPERATOR_TOKEN` are gone from the console, `.env.example`, the compose file and the installer. Two live sessions survived the restart, because a session is a row rather than something the process remembers. The service-credential half was never built and the reason is recorded — nothing automated ever used the token |
