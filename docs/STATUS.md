...
| 0053e | Cloud.AI | The shared token retired | **open** | **Half the premise was wrong.** Nothing automated uses the shared token: the installer only generates it and the revocation-list publisher reads `/pki/`, which is public. There is nobody to issue a named service credential to, so that half is not built. The removal is blocked on the console, which signs in by pasting the header and has no sign-in page. What landed: operator actions record **who** — a signed-in username, or `shared-token` where the shared secret was used — and **the console now has a sign-in screen**, with a route guard sending anybody without a session to it. The store sends the session first and falls back to the header, which is the last thing left to delete |
