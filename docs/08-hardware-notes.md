...
| **A sweep and a job compete for the same card** | An inventory read on a 5-second poll and an enrolment on the same reader produced `0x8010000B SCARD_E_SHARING_VIOLATION`: the job failed instantly, before any prompt, with the agent's own housekeeping as the cause | [03 § One reader](03-piv-layer.md), `AgentWorker.PollAsync` |
| **A fingerprint match does not satisfy a key generated with PIN policy `Once`** | `VERIFY 96` answered `9000` and the slot then refused to sign with `6982`. The Bio's `MatchOnce` (`0x04`) is set at generation and cannot be changed after; two slots on one token, `Once` failing and `MatchOnce` working, are the evidence | [03 § Biometric verification](03-piv-layer.md), `CardEnrolment` |
| **Biometric verification is `VERIFY 96` with an empty TLV `03`; `02` returns a temporary PIN** | Measured with a finger on the sensor: `03 00` → `9000`, `02 00` → `9000` plus sixteen bytes, `00` → `6A80` and nothing consumed | [03 § Biometric verification](03-piv-layer.md), `PivBiometricOperations` |
| **One unresponsive reader ends an operation aimed at a different token** | An OMNIKEY 5022 answered `0x80100066` and took down an enrolment meant for a YubiKey two readers along, in three tenths of a second | `CardEnrolment`, `CardOperations`, `PukUnblock` |
...
| **CRS 930120 matches argument _names_, so a field called `profileName` is a 403** | `Matched Data: .profile found within ARGS_NAMES:json.profileName` — `.profile` is a Unix dotfile in `lfi-os-files.data`, and `PmFromFile` matches on substring, so renaming does not escape it | [06 § What the WAF costs](06-security.md), `docker/edge/default.conf.template` |
| **`X509Certificate2.CreateFromPemFile(path)` demands a private key in the same file** | Pinning a CA certificate threw "the key does not match the certificate" — the single-argument overload looks for the key alongside the certificate, so the pinned-root option had never once worked | `BackendClient` |
| **State read before an action and reported after it overwrites the action** | The agent posted a sweep taken before the enrolment: 50ms after the server recorded slot 9E provisioned, the stale report recorded it empty, while the certificate sat on the card | [03 § One reader](03-piv-layer.md), `AgentWorker.PollAsync` |
| **A smart-card logon profile must refuse to issue without a resolved SID** | The first end-to-end enrolment was refused with 422 rather than producing a certificate a domain controller would reject at logon; the lab uses a `client-auth` profile that does not claim to be a logon credential | [04](04-pki-backends.md), `Profiles` |
