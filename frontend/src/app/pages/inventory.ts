...
export class Inventory {
  protected readonly store=inject(ConsoleStore); protected readonly i18n=inject(I18n); private readonly route=inject(ActivatedRoute); protected readonly query=signal(''); protected readonly busy=signal<string|null>(null); protected readonly message=signal<string|null>(null);
  ...
  protected readonly columns=computed(()=>({tokens:[this.i18n.t('serial'),this.i18n.t('model'),this.i18n.t('firmware'),this.i18n.t('pin'),this.i18n.t('status')],certificates:[this.i18n.t('subject'),this.i18n.t('tokenSlot'),this.i18n.t('validTo'),this.i18n.t('status'),this.i18n.t('actions')],agents:[this.i18n.t('hostname'),this.i18n.t('domain'),this.i18n.t('version'),this.i18n.t('lastSeen'),this.i18n.t('status')],jobs:[this.i18n.t('operation'),this.i18n.t('token'),this.i18n.t('attempt'),this.i18n.t('created'),this.i18n.t('status')]})[this.kind]);
  protected readonly rows=computed(()=>{ const s=this.store.snapshot(); let rows:{id:string;stateIndex:number;cells:string[];tokenSerial?:number;slotId?:string}[];
    ...
    else if(this.kind==='certificates') rows=s.credentials.map(x=>({id:x.id,stateIndex:3,tokenSerial:x.tokenSerial,slotId:x.slotId,cells:[x.subjectDn??'—',`${x.tokenSerial} / ${x.slotId}`,x.notAfter?new Date(x.notAfter).toLocaleDateString():'—',x.state]}));
  ...
  protected async recycle(row:{id:string;tokenSerial?:number;slotId?:string}):Promise<void> {
    if(row.tokenSerial===undefined||!row.slotId||!confirm(this.i18n.t('recycleConfirm'))) return;
    this.busy.set(row.id); this.message.set(null);
    try { await this.store.recycleCredential(row.tokenSerial,row.slotId); this.message.set(this.i18n.t('jobQueued')); }
    catch { this.message.set(this.i18n.t('operationFailed')); }
